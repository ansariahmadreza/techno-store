"use client"
import { RootCarousel } from "./helper/SwiperCar";
        ...
        <div>
            <Container>
                <section className="rounded-2xl border-neutral-300 px-2  py-6 mx-auto  border mt-10 ">
                    ...
                    >
                        {Carousel.map((item) =>
                            <SwiperSlide key={item.id} className="border-r sm:h-217.5   border-neutral-300 px-2 py-4  mt-15 relative">
                                <Link href={`/${item.id}`}>
                                    ...
                                    <img loading="lazy" className=" rounded-sm w-[180] h-35  border-red-500 border-t-5 mx-auto cursor-pointer "
                                        src={item.image} alt={item.description} title={item.description} />
                                    <p className="h-22.5  py-2 mx-auto px-1">{item.description}</p>

                                    <div className="flex items-center justify-between ">
                                        <p className="pr-37.25 md:pr-28 float-left">{formatNumberWithCommas(item.DiscountedPrice)}</p>
                        ...
                        <section>
                            <div className="left-btn  mr-2 top-[50%] absolute  z-40 w-7.5 h-7.5 text-black bg-neutral-100
                     rounded-2xl text-2xl pt-0.5  ">
                                <AiOutlineRight className="cursor-pointer hover:text-black text-neutral-500" />
                            </div>
                        </section>
                        <section className="flex justify-end items-center">
                            <div className="Right-btn  top-[50%] absolute h-7.5 rounded-2xl  z-40 bg-neutral-100  shadow-2xl
                    w-7.5 text-2xl pt-0.5 pr-1  group-hover ">
