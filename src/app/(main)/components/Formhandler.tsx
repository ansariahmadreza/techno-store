    ...
    return (
        <div>
            <form className="flex flex-col items-center justify-center gap-3 pt-7" onSubmit={handleSubmit(onSubmit)} >
                <input type="text" {...register('name')} className="outline-0 border rounded p-1 w-75" placeholder="نام و نام خانوادگی" />
                {errors.name && <p className="text-red-500 text-sm">{errors.name.message}</p>}
                
                <input type="email" {...register("email")} className="outline-0 border rounded p-1 w-75" placeholder="ایمیل" />
                {errors.email && <p className="text-red-500 text-sm">{errors.email.message}</p>}
                
                <input type="password" placeholder="رمز عبور" className="outline-0 border rounded p-1 w-75" {...register("password")} />
                {errors.password && <p className="text-red-500 text-sm">{errors.password.message}</p>}
                
                <input type="password" {...register("confirmPassword")} className="outline-0 border rounded p-1 w-75" placeholder="تکرار رمز عبور" />
                ...
                <button type="submit"
                    ...
                    className={`p-2 text-white rounded-lg cursor-pointer mt-9 w-75 transition-colors ${(isSubmitting || isLoading) ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}
                >
